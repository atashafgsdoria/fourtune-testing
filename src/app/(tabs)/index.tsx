import PatientDetailScreen from '@/src/app/patient/[id]';
...
import { getAuthDisplayName } from '@/src/utils/authDisplayName';
import {
  borderRadius,
  colors,
  fonts,
...
export default function HomeScreen() {
  ...
  if (profile?.role === 'Patient') {
    if (!profile.patient_id) {
      return (
        <SafeAreaView style={styles.screen}>
          <View style={styles.patientLinkError}>
            <Text style={styles.patientLinkErrorText}>
              Your patient profile is not linked yet.
            </Text>
          </View>
        </SafeAreaView>
      );
    ...
    return <PatientDetailScreen patientId={profile.patient_id} patientView />;
  }

  return <WorkerHomeScreen user={user} />;
}

function WorkerHomeScreen({ user }: { user: User | null }) {
  ...
  const displayName = getAuthDisplayName(user?.user_metadata, user?.email);
