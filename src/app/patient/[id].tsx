...
import {
    getOpenCrPatientById,
    getPatientById,
    getPatientByInternalId,
} from '@/src/fhir/fhirClient';
...
export default function PatientDetailScreen({
  patientId,
  patientView: patientViewProp = false,
}: PatientDetailProps = {}) {
  ...
  const isPatientView = patientViewProp || params.patientView === 'true';
  ...
  const loadPatient = useCallback(async () => {
    ...
    try {
      ...
      if (resource) {
        ...
        // Remote fallback (e.g. fresh install with empty SQLite).
        // OpenCR is the canonical patient store (write path syncs Patients there).
        // The value in Supabase users.patient_id is the local id / internalid, so
        // the identifier search is the primary, proven lookup:
        //   GET /opencr/fhir/Patient?identifier=...internalid|{id}  → 200
        // HAPI direct read is kept as a last resort for any legacy record that only
        // exists in the main HAPI store.
        console.log(`PATIENT READ (remote): looking up ${id}`);
        let remotePatient: FHIRPatient;
        try {
          remotePatient = await getPatientByInternalId(id);
          console.log(`PATIENT READ (remote): resolved via OpenCR internalid`);
        } catch (internalIdError) {
          console.warn(
            `PATIENT READ (remote): OpenCR internalid lookup failed`,
            internalIdError
          );
          try {
            remotePatient = await getOpenCrPatientById(id);
            console.log(`PATIENT READ (remote): resolved via OpenCR id`);
          } catch (openCrError) {
            console.warn(`PATIENT READ (remote): OpenCR id lookup failed`, openCrError);
            remotePatient = await getPatientById(id);
            console.log(`PATIENT READ (remote): resolved via HAPI FHIR (legacy)`);
          }
        }
      ...
      >
        ...
        <View style={styles.profileCard}>
          ...
          <Text style={styles.profileName}>{fullName}</Text>
            {!isPatientView && <View style={styles.profileMetaRow}>
